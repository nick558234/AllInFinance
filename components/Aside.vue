<template>
  <aside class="w-64 bg-[#2C83EC] text-white flex flex-col">
    ...
    <nav class="flex-1 p-4">
      <ul>
        <li v-for="category in categories" :key="category.id" class="mb-4">
          <button @click="selectCategory(category.id)" class="hover:bg-[#333333] font-bold text-xl uppercase hover:text-white p-2 rounded block">{{ category.name }}</button>
        </li>
      </ul>
    </nav>
  </aside>
